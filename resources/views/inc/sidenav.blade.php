<ul id="slide-out" class="sidenav sidenav-fixed grey lighten-4">
    <li>
        <div class="user-view">
           ...
           {{--  <a href="{{route('admin.auth.show')}}"><img class="circle" src="{{asset('storage/admins/'.Auth::admin()->picture)}}"></a> --}}
            {{-- Get first and last name of authenicated user --}}
            <a href="{{route('admin.auth.show')}}"><span class="white-text name">{{-- {{ Auth::admin()->first_name }} {{ Auth::admin()->last_name }} --}}</span></a>
            {{-- Get email of authenicated user --}}
            <a href="{{route('admin.auth.show')}}"><span class="white-text email">{{-- {{ Auth::admin()->email }} --}}</span></a>
        </div>
    </li>
    <li>
        <a class="waves-effect waves-grey" href="{{route('admin.dashboard')}}"><i class="material-icons">dashboard</i>Dashboard</a>
    </li>
    <li>
